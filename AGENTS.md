...
- **`tesla.cost` is a cross-repo contract.** Emitted beside `signal_count` at `trackSignalUsage`: each accepted `V`/`alerts`/`errors` record adds `signals / 150000` US dollars (Tesla sells 150,000 streaming signals per US$1) against the connection-authenticated VIN. Its name, unit `USD`, and its exactly two attributes - `teslemetry.cost.type` (`streaming_data`/`streaming_alerts`/`streaming_errors`) and `teslemetry.cost.id` (the VIN) - are shared with the api repo's other `tesla.cost` sources (vehicle_data, commands, wakes, energy) so every source sums into one per-vehicle figure - changing any of them is a breaking cross-service change. Connectivity records are server-generated and unbilled.
