...
Our release path is `gh release create` on `main`, which triggers `.github/workflows/release-binary.yml` to build and attach the `linux-amd64` binary + sha256 that production deploys actually consume. There is deliberately no `publish.yml` — upstream's version of that file (inherited when this fork was created) pushed Docker images to `tesla/fleet-telemetry` on Docker Hub (not ours; always failed here on missing `DOCKERHUB_*` secrets) and cut GitHub releases via the built-in Actions token, which does *not* trigger `release-binary.yml`. That silently shipped a release with zero binary assets on 2026-07-06 (fork run 28761905304, its only run, which failed) and stalled a rollout. Don't recreate a `publish.yml`-style workflow; if Docker image publishing is ever wanted again, it needs to shell out to (or itself trigger) `release-binary.yml`'s asset-producing path rather than creating a parallel release.
